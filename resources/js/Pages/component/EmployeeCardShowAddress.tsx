...
type EmployeesaddressTypes={
    codigo: string;
    usuario: string;
    street_employee_address: string;
    number_employee_address:string;
    neighborhood_employee_address:string;
    postal_code_employee_address:string;
    city_employee_address: string;
    state_employee_address:string;
    cod_user: string;
}

export default function EmployeeCardShowAddress({codigo,usuario}:EmployeesaddressTypes){
    const [address, setAddress] = useState<EmployeesaddressTypes[]>([]);
    const [loading, setLoading] = useState(true); // Adicione um estado para rastrear o carregamento
    useEffect(() => {
        fetch(`/cardshowaddressemployee/${codigo}`) // Rota da API Laravel
    ...
    //
        ...
        <div className="max-w-7xl mx-auto sm:px-6 mt-5 lg:px-8 space-y-6">
            ...
            <p className="text-2xl font-bold">Dados do endereço </p>
                {address.length === 0 ? (
                    ...
                    <div className="">
                       <div className="flex items-center shadow p-4 mb-4 text-sm text-yellow-800 rounded-lg bg-yellow-50 dark:bg-gray-800 dark:text-yellow-300" role="alert">
                        ...
                        <div>
                            <span className="font-medium">Atenção!</span> Não existe dados de endereço cadastrado para esse usuário.
                        </div>
                        </div>
                    </div>
                    </>
                ...
                <>
                    <div>
                        {address.map((endereco,item)=>(<>
                        <div className="flex justify-between p-2">
                            <div className="t">
                                <p className=''>
                                    Logradouro:&nbsp;{endereco.street_employee_address}, {endereco.number_employee_address}
                                </p>
                                <p className=''>
                                    Bairro: {endereco.neighborhood_employee_address}
                                </p>
                                <p className=''>
                                    CEP: {endereco.postal_code_employee_address.replace(/(\d{5})(\d{3})/, "$1-$2")}
                                </p>
                                <p className=''>
                                Cidade:&nbsp;{endereco.city_employee_address}, {endereco.state_employee_address}
                                </p>
                            </div>
                            <div className="flex">
                                <>
                                <div className="inline-flex rounded-md shadow-sm" role="group">
                                    {address === null ? (
                                        <>Nulo</>
                                            ...
                                            <>
                                            <div className="t">
                                                <a href={`../../formecadastrarenderecofuncionario/${endereco.cod_user}`} className="inline-flex px-4 py-2 text-sm font-medium text-gray-900 bg-white border border-gray-200 rounded-l-lg hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-2 focus:ring-blue-700 focus:text-blue-700 dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-blue-500 dark:focus:text-white print:hidden">
